...
int dp[N][N];
int f(int i,int secondIndex){
    if(i>=n)    return 0;

    if(dp[i][secondIndex]!=-1)  return dp[i][secondIndex];

    int init=preCalc[secondIndex];
    int o1=f(i+1,secondIndex+1);
    int o2=init*arr[i]+f(i+1,secondIndex);
    return dp[i][secondIndex]=max(o1,o2);
}
...
void solve(){
    ...
    preCalc[0]=k;
    FOR(i,1,n)  preCalc[i]=preCalc[i-1]+pow(sumOfDigits(preCalc[i-1]),3);
