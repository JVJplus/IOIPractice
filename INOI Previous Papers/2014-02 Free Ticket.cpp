...
void allPairShortestPath(){
    FOR(j,1,n){
        FOR(i,1,n){
            FOR(k,1,n){
                srtPath[i][k]=min(srtPath[i][k],srtPath[i][j]+srtPath[j][k]);
          }
      }
  }
}
...
void solve(){
    cin>>n>>m;
    For(i,N)For(j,N){adj[i][j]=INF;srtPath[i][j]=INF;}
    ...
    allPairShortestPath(); //Floydd Warshall Algorithm
     //Floyd Warshall Algorithm (Not Fear with the name, its just a simple brute force algo). 
    /*I still remember how much i fear from this algorithm just by seeing that this uses DP.
    It's really a great feeling understanding it. :D
    */
    int mx=0;
    FOR(i,1,n){
        FOR(j,1,n){
