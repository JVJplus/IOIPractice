...
#define N 1000056
ll n,k;
ll a[N],b[N],pref[N],diff[N],sum[N];
ll mx=-INF;

void solve(){
    cin>>n;
    For(i,n){    cin>>a[i];    REMAX(mx,a[i]); } //1st case i==j handled
    For(i,n)     cin>>b[i];    

    pref[0]=b[0];
    FOR(i,1,n-1) pref[i]=pref[i-1]+b[i];

    diff[0]=a[0]-pref[0];
    FOR(i,1,n-1)  diff[i]=max(diff[i-1],a[i]-pref[i]);

    sum[0]=a[0];
    FOR(i,1,n-1)  sum[i]=max(sum[i-1],a[i]+pref[i-1]);

    //2nd case i<j
    FOR(i,1,n-1) REMAX(mx,  (  diff[i]+a[i]+pref[i-1]   ) );   

    //3rd case i>j
    FOR(i,1,n-1) REMAX(mx,  (  pref[n-1]+ sum[i-1]+ a[i]-pref[i]  ) );   
